...
const ALLOWED_WHO = new Set(['Sumit', 'Sanya'])
...
export default function CoupleQuestions() {
    ...
    // Structure: { Sumit: { answer, date }, Sanya: { answer, date } }
    useEffect(() => {
        // Clear draft on identity/day change (prevents Sumit's text leaking to Sanya)
        setMyAnswer('')
        ...
        const unsubToday = onSnapshot(todayRef, snap => {
            ...
            const mine = data[who]
            const partner = data[who === 'Sumit' ? 'Sanya' : 'Sumit']
            if (mine?.answer) {
        ...
        const unsubPast = onSnapshot(pastQ, snap => {
            ...
            snap.docs.forEach(d => {
                ...
                const answers = []
                if (data.Sumit?.answer) answers.push({ who: 'Sumit', answer: data.Sumit.answer, date: data.Sumit.date })
                if (data.Sanya?.answer) answers.push({ who: 'Sanya', answer: data.Sanya.answer, date: data.Sanya.date })
    ...
    const submit = async () => {
        ...
        if (isVisitor) return showToast('Only Sumit & Sanya can answer 🔒')
    ...
    const partner = who === 'Sumit' ? 'Sanya' : 'Sumit'
                ...
                <>
                    ...
                    <div style={s.statusRow}>
                        <div style={{ ...s.pill, background: submitted ? '#e8f5e9' : '#fdf0f5', color: submitted ? '#2e7d32' : 'var(--mauve)' }}>
                            {who === 'Sumit' ? '💙' : '🌸'} {who}: {submitted ? 'Answered ✓' : 'Not answered'}
                        </div>
                        <div style={{ ...s.pill, background: partnerAnswer ? '#e8f5e9' : '#fdf0f5', color: partnerAnswer ? '#2e7d32' : 'var(--mauve)' }}>
                            {who === 'Sumit' ? '🌸' : '💙'} {partner}: {partnerAnswer ? 'Answered ✓' : 'Waiting…'}
                        </div>
                    </div>
                    ...
                    {!isVisitor && (
                        <div style={s.answerSection}>
                            <div className="section-label">Your Answer {who === 'Sumit' ? '💙' : '🌸'}</div>
                            <textarea
                                value={myAnswer}
                            ...
                            >
                                ...
                                {partnerAnswer ? (
                                    <div style={s.partnerAnswerCard}>
                                        <div style={s.partnerAnswerIcon}>{who === 'Sumit' ? '🌸' : '💙'}</div>
                        ...
                        <div key={group.key} style={{ ...s.pastGroup, animationDelay: `${Math.min(i * 0.05, 0.4)}s` }}>
                            ...
                            {group.answers.map(a => (
                                <div key={a.who} style={s.pastAnswer}>
                                    <span style={s.pastWho}>{a.who === 'Sumit' ? '💙' : '🌸'} {a.who}</span>
