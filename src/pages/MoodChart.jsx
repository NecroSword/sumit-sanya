...
export default function MoodChart() {
    ...
    // Per-person split
    const sumitMoods = filtered.filter(m => m.who === 'Sumit')
    const sanyaMoods = filtered.filter(m => m.who === 'Sanya')
    ...
    const timelineData = timelineDays.map(dayStr => {
        ...
        const sumitCount = dayMoods.filter(m => m.who === 'Sumit').length
        const sanyaCount = dayMoods.filter(m => m.who === 'Sanya').length
        const d = new Date(dayStr)
        return { label: SHORT_DAY[d.getDay()], sumitCount, sanyaCount, total: dayMoods.length }
    })
    const maxBarDay = Math.max(1, ...timelineData.map(d => d.total))

    // Most recent mood per person
    const lastSumit = sumitMoods[0]
    const lastSanya = sanyaMoods[0]

    // Mood sync (do they feel the same today?)
    const todayStr = new Date().toDateString()
    // moods is already sorted newest-first from the listener — just filter
    const lastSumitToday = moods.find(m => m.who === 'Sumit' && new Date(m.date).toDateString() === todayStr)?.emoji
    const lastSanyaToday = moods.find(m => m.who === 'Sanya' && new Date(m.date).toDateString() === todayStr)?.emoji
    const inSync = lastSumitToday && lastSanyaToday && moodSentiment(lastSumitToday) === moodSentiment(lastSanyaToday)
        ...
        <div className="page-content">
                ...
                <div style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-around' }}>
                        <div style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: '2rem' }}>{lastSumitToday || '—'}</div>
                            <div style={{ fontSize: '0.7rem', color: 'var(--text-light)' }}>💙 Sumit</div>
                        </div>
                        <div style={{ fontSize: '1.2rem', alignSelf: 'center', opacity: 0.4 }}>vs</div>
                        <div style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: '2rem' }}>{lastSanyaToday || '—'}</div>
                            <div style={{ fontSize: '0.7rem', color: 'var(--text-light)' }}>🌸 Sanya</div>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div style={S.twoCol}>
                <div style={S.lastMoodCard}>
                    <div style={S.lastWho}>💙 Sumit</div>
                    {lastSumit
                        ? <><div style={S.lastEmoji}>{lastSumit.emoji}</div><div style={S.lastLabel}>{MOOD_LABELS[lastSumit.emoji] || lastSumit.emoji}</div><div style={S.lastTime}>{new Date(lastSumit.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</div></>
                        : <div style={{ color: 'var(--text-light)', fontSize: '0.78rem', marginTop: 8 }}>No mood yet</div>
                    }
                </div>
                <div style={S.lastMoodCard}>
                    <div style={S.lastWho}>🌸 Sanya</div>
                    {lastSanya
                        ? <><div style={S.lastEmoji}>{lastSanya.emoji}</div><div style={S.lastLabel}>{MOOD_LABELS[lastSanya.emoji] || lastSanya.emoji}</div><div style={S.lastTime}>{new Date(lastSanya.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</div></>
                        : <div style={{ color: 'var(--text-light)', fontSize: '0.78rem', marginTop: 8 }}>No mood yet</div>
                    }
                </div>
            </div>
            ...
            {range !== 'all' && (
                <Card>
                    ...
                    <div style={{ display: 'flex', gap: 14, justifyContent: 'center', marginTop: 8 }}>
                        <span style={S.legend}><span style={{ ...S.legendDot, background: 'var(--mauve-deep)' }} />💙 Sumit</span>
                        <span style={S.legend}><span style={{ ...S.legendDot, background: 'var(--rose-dark)' }} />🌸 Sanya</span>
                    </div>
                </Card>
            )}
