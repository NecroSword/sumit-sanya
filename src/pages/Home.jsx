...
const ANNIVERSARY = new Date('2026-02-07T00:00:00')
...
export default function Home() {
  ...
  const [gmData, setGmData] = useState(null) // { sumitGM, sanyaGM, sumitGN, sanyaGN }
  ...
  // Miss you counter
  const [missCount, setMissCount] = useState({ Sumit: 0, Sanya: 0 })
  ...
  const logMood = async (emoji) => {
    if (isVisitor) return showToast('Only Sumit & Sanya can do this 🔒')
    setLogging(true)
    try {
  ...
  const sendHugOrKiss = async (type) => {
    ...
    try {
      ...
      showToast(`${emoji} Sent to ${who === 'Sumit' ? 'Sanya' : 'Sumit'}! ${emoji}`)
    } catch (err) {
  ...
  const tapMissYou = async () => {
    ...
    try {
      ...
      notifyPartner(who, {
        ...
        body: `${who === 'Sumit' ? 'Sanya' : 'Sumit'}, you're on ${who}'s mind 💕`,
        url: '/',
      }).catch(() => { })
    } catch (err) {
  ...
  const partnerSentGM = gmData?.[`${who === 'Sumit' ? 'Sanya' : 'Sumit'}GM`]
  const partnerSentGN = gmData?.[`${who === 'Sumit' ? 'Sanya' : 'Sumit'}GN`]
  ...
  return (
    <div className="page-content">
      ...
      <div style={styles.hero}>
        ...
        <div style={styles.heroNames}>Sumit &amp; Sanya</div>
        <div style={styles.heroSince}>Together since February 7, 2026</div>
      ...
      {!isVisitor && (
        <Card>
          ...
          <div style={styles.missCounter}>
            <span style={styles.missItem}>💙 Sumit missed: {missCount.Sumit || 0}x</span>
            <span style={styles.missItem}>🌸 Sanya missed: {missCount.Sanya || 0}x</span>
          </div>
        </Card>
      )}
